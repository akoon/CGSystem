 ...
 */
 ...
 Ext.define("Gnt.plugin.HtmlEditor", {  
...
Ext.define("Sch.column.timeAxis.Horizontal", {
    ...
    createColumns: function(e, g, c) {
        ...
        m = g.bottom || g.middle,
        j = e.getTicks(),
        h = Sch.mixin.TimelineView.Config.timeCellCls,
        k;
        for (var d = 0,
        a = j.length; d < a; d++) {
            k = {
            ...
            if (m.renderer) {
                k.header = m.renderer.call(m.scope || this, j[d].start, j[d].end, k, d)
            } else {
                k.header = this.defaultRenderer(j[d].start, j[d].end, m.dateFormat)
            }
            b[b.length] = Ext.create("Sch.column.Time", Ext.apply(k, c))
        }
...
Ext.define("Sch.mixin.Lockable", {
    ...
    processColumns: function(b) {
        ...
        function(d) {
            ...
            if (d.locked && d.hidden && Ext.versions.extjs.isLessThan("4.0.7")) {
    ...
    onSchedulerGridAfterHeaderLayout: function() {
        var c = this.lockedGrid.headerCt;
        ...
        if (Ext.versions.extjs.isGreaterThan("4.0.5")) {
            ...
            c.items.each(function(d) {
                if (d.rendered) {
                    d.el.down(".x-column-header-inner").setHeight("auto")
                }
            });
            c.layout.align = "stretch";
            c.height = undefined;
            delete this.lockedGrid.componentLayout.lastComponentSize;
            c.setSize(c.getWidth(), a.getHeight());
            c.items.each(function(d) {
                if (d.rendered) {
                    d.setPadding()
                }
...
Ext.define("Sch.Overrides", {
    requires: ["Ext.tree.Panel", "Ext.tree.View", "Ext.data.NodeInterface", "Ext.data.reader.Xml", "Ext.data.TreeStore", "Ext.grid.feature.Grouping", "Ext.container.Viewport", "Ext.grid.plugin.Editing", "Ext.grid.plugin.CellEditing", "Sch.mixin.Lockable"]
},
function() {
    Ext.Base.prototype.statics = function() {
        var c = this.statics.caller,
        b = this.self;
        if (!c) {
            return b
        }
        return c.$owner || c.caller.$owner
    };
    Ext.grid.plugin.Editing.override({
        startEditByClick: function(d, b, j, c, h, f, g) {
            var i = d.getHeaderAtIndex(j);
            if (i.getEditor && i.getEditor(c)) {
                this.startEdit(c, i)
            }
        }
    });
    Ext.grid.plugin.CellEditing.override({
        getEditor: function(b, c) {
            if (!c.getEditor) {
                return false
            }
    ...
    Ext.panel.Table.override({
        elScroll: function(e, f, c) {
            ...
            if (e === "down" || e === "up") {
                b = d.getVerticalScroller();
                b && b.scrollByDeltaY(f)
            } else {
                b = d.getHorizontalScroller();
                b && b.scrollByDeltaX(f)
            }
        }
    });
    ...
    Ext.data.TreeStore.override({
        onNodeAdded: function(e, f) {
            ...
            if (!f.isLeaf() && !f.isLoaded()) {
                h = b.getRoot(g);
                if (h) {
                    this.fillNode(f, b.extractData(h));
                    if (g[b.root]) {
                        delete g[b.root]
                    }
                }
    ...
    Ext.panel.Table.override({
        scrollByDeltaX: function(b) {
            var c = this.getHorizontalScroller();
            if (c) {
                c.scrollByDeltaX(b)
            }
        }
    ...
    if (Ext.isIE9 && Ext.isStrict) {
        Sch.mixin.Lockable.override({
            onNormalGridAfterRefresh: function() {
                var f = this,
                b = f.normalGrid.getView(),
                d = b.el,
                g = d.query(b.getItemSelector()),
                e = g.length,
                c = 0;
                f.normalHeights = [];
                for (; c < e; c++) {
                    f.normalHeights[c] = g[c].offsetHeight
                }
                f.syncRowHeights()
            },
            onNormalGridAfterUpdate: function(b, c, d) {
                this.lockedGrid.getView().onUpdate(this.lockedGrid.store, b);
    ...
    Ext.grid.feature.Grouping.override({
        expand: function(d) {
            ...
            e = c.up("gridpanel"),
            b = Ext.getDom(d),
            g = b.id.match(c.id + "-gp-(.*)")[1];
            f.collapsedState[b.id] = false;
            d.removeCls(f.collapsedCls);
            d.prev().removeCls(f.hdCollapsedCls);
            e.determineScrollbars();
            e.invalidateScroller();
            c.fireEvent("groupexpand", c, g)
        },
        collapse: function(d) {
            var f = this,
            c = f.view,
            ...
            b = Ext.getDom(d),
            g = b.id.match(c.id + "-gp-(.*)")[1];
            f.collapsedState[b.id] = true;
            d.addCls(f.collapsedCls);
            d.prev().addCls(f.hdCollapsedCls);
            e.determineScrollbars();
            e.invalidateScroller();
            c.fireEvent("groupcollapse", c, g)
        }
    });
    Ext.tree.View.override({
        ...
        initComponent: function() {
            ...
            b.store = b.providedStore || Ext.create("Ext.data.NodeStore", Ext.apply({
                recursive: true,
    ...
    if (Ext.versions.extjs.isGreaterThan("4.0.5")) {
        Ext.LoadMask.override({
            onBeforeLoad: function() {
                var c = this,
                b = c.ownerCt || c.floatParent;
                if (!this.disabled) {
                    if (b.componentLayoutCounter) {
                        Ext.Component.prototype.show.call(c)
                    } else {
                        b.afterComponentLayout = Ext.Function.createSequence(b.afterComponentLayout,
                        function() {
                            if (!c.hidden) {
                                Ext.Component.prototype.show.call(c)
                            }
                            delete b.afterComponentLayout
                        })
                    }
                }
            }
    ...
    if (Ext.versions.extjs.isGreaterThan("4.0.6")) {
        Ext.data.TreeStore.override({
            load: function(c) {
                c = c || {};
                c.params = c.params || {};
                var e = this,
                d = c.node || e.tree.getRootNode(),
                b;
                if (!d) {
                    d = e.setRootNode({
                        expanded: true
                ...
                c.params[e.nodeParam] = d ? d.getId() : "root";
                if (d) {
                    d.set("loading", true)
                }
                return e.callParent([c])
            }
        })
    }
    Ext.container.Viewport.override({
        initComponent: function() {
            this.callOverridden();
            this.el.addCls(Ext.baseCSSPrefix + "reset")
        }
    });
    if (Ext.versions.extjs.isGreaterThan("4.0.5")) {
        Ext.data.Operation.override({
            commitRecords: function(c) {
                var h = this,
                i, d, e, g, b;
                if (!h.actionSkipSyncRe.test(h.action)) {
                    e = h.records;
                    if (e && e.length) {
                        i = Ext.create("Ext.util.MixedCollection", true,
                        function(j) {
                            return j.getId()
                        });
                        i.addAll(e);
                        for (d = c ? c.length: 0; d--;) {
                            g = c[d];
                            b = i.get(g.getId());
                            var f = {};
                            g.fields.each(function(j) {
                                if (j.persist) {
                                    f[j.name] = g.data[j.name]
                                }
                            });
                            if (b) {
                                b.beginEdit();
                        ...
                        if (h.actionCommitRecordsRe.test(h.action)) {
                            for (d = e.length; d--;) {
                                e[d].commit()
                            }
                        }
    ...
    Ext.data.NodeInterface.getPrototypeBody = function() {
        ...
        Ext.apply(b, {
            updateInfo: function(o) {
                ...
                e = p.isRoot(),
                m = p.parentNode,
                g = (!m ? true: m.firstChild == p),
                l = (!m ? true: m.lastChild == p),
                k = 0,
                r = p,
                d = p.childNodes,
                n = d.length,
                j = 0,
                h = p[p.persistenceProperty];
                while (r.parentNode) {++k;
                    r = r.parentNode
                }
                var q = {
                    isFirst: g,
                ...
                while (f && f.get("index") !== c.get("index") + 1) {
                    if (o) {
                        f[f.persistenceProperty].index = c.get("index") + 1
                    } else {
    ...
    Sch.mixin.TimelineView.Config = {
        ...
        fitColumns: function(b) {
            ...
            if (this.orientation === "horizontal") {
                ...
                a = Math.floor((this.panel.getWidth() - Ext.getScrollBarWidth() - 1) / this.headerCt.getColumnCount())
            }
...
Ext.require(["Ext.tree.View", "Sch.mixin.TimelineView", "Sch.util.Mixin"],
function() {
...
function() {
    Sch.mixin.TimelinePanel.Config = {
        ...
        switchViewPreset: function(d, a, g, b) {
            if (this.fireEvent("beforeviewchange", this, d, a, g) !== false) {
                if (Ext.isString(d)) {
                    this.viewPreset = d;
                ...
                var f = d.headerConfig;
                var c = {
                    unit: f.bottom ? f.bottom.unit: f.middle.unit,
                    increment: (f.bottom ? f.bottom.increment: f.middle.increment) || 1,
                    resolutionUnit: d.timeResolution.unit,
                    resolutionIncrement: d.timeResolution.increment,
                    weekStartDay: this.weekStartDay,
                    mainUnit: f.middle.unit,
                ...
                if (e && e.headerCt && Ext.versions.extjs.isGreaterThan("4.0.5")) {
                    e.headerCt.on("afterlayout",
                    function() {
                        var i = this.lockedGrid.headerCt;
                        var h = e.headerCt;
                        i.setSize(i.getWidth(), h.getHeight())
                    },
                    this, {
                        single: true
                    })
        ...
        afterRender: function() {
            ...
            if (!this.lockable) {
                ...
                if (b && b.reorderer && b.reorderer.dropZone) {
                    ...
                    a.positionIndicator = Ext.Function.createSequence(a.positionIndicator,
                    function() {
                        this.valid = false
                    })
                }
...
Ext.require(["Sch.mixin.TimelinePanel", "Sch.util.Mixin"],
function() {
    		...
    		'<html class="x-border-box {htmlClasses}">'+
				...
				'<body class="sch-print-body {bodyClasses}">'+
					'<div class="sch-print-ct {componentClasses}" style="width:{totalWidth}px">'+
						'<div class="sch-print-headerbg" style="border-left-width:{totalWidth}px;height:{headerHeight}px;">'+
						'</div>'+
						...
						'<div style="width:{lockedWidth}px;top:{headerHeight}px;" class="sch-print-locked-rows-ct {innerLockedClasses} x-grid-inner-locked">'+
							'{lockedRows}'+
						'</div>'+
						'<div style="left:{[Ext.isIE ? values.lockedWidth : 0]}px;top:{headerHeight}px;width:{normalWidth}px"'+
						'class="sch-print-normal-rows-ct {innerNormalClasses}">'+
							'{normalRows}'+
						'</div>'+
					'</div>'+
...
Ext.define("Gnt.model.CalendarDay", {
    ...
    fields: [{
        ...
        dateFormat: "c",
        convert: function(a) {
            var b = Ext.data.Types.DATE.convert.call(this, a);
            return Ext.Date.clearTime(b)
        }
    },
    {
        name: "IsWorkingDay",
...
Ext.define("Gnt.model.Task", {
    ...
    isManuallyScheduled: function() {
        return this.get("ManuallyScheduled")
    },
    setStartDate: function(a, e, d) {
        this.beginEdit();
        var c = this.getCalendar();
        if (d && !this.get("ManuallyScheduled")) {
            if (!this.isMilestone() || c.isHoliday(a - 1)) {
                a = c.skipNonWorkingTime(a, true)
            }
        }
        if (e !== false) {
            var b = this.get("Duration");
            if (Ext.isNumber(b)) {
                this.set({
                    StartDate: a,
                    EndDate: this.calculateEndDate(a, b, this.get("DurationUnit"))
                })
            } else {
                this.set("StartDate", a)
            }
        } else {
            if (this.getEndDate()) {
                this.set({
    ...
    setEndDate: function(a, e, d) {
        ...
        var c = this.getCalendar();
        if (d && !this.get("ManuallyScheduled")) {
            a = c.skipNonWorkingTime(a, false)
        }
        if (e !== false) {
            var b = this.get("Duration");
            if (Ext.isNumber(b)) {
                this.set({
                    StartDate: this.calculateStartDate(a, b, this.get("DurationUnit")),
                    EndDate: a
                })
            } else {
                this.set("EndDate", a)
    ...
    recalculateParents: function() {
        ...
        if (e && !e.isRoot() && e.childNodes.length > 0) {
            if (!e.isManuallyScheduled()) {
                for (var d = 0,
                a = e.childNodes.length; d < a; d++) {
                    var f = e.childNodes[d];
                    b = Sch.util.Date.min(b, f.getStartDate() || b);
                    c = Sch.util.Date.max(c, f.getEndDate() || c)
                }
                e.beginEdit();
                var h = b - new Date(9999, 0, 0) !== 0 && e.getStartDate() - b !== 0,
                g = c - new Date(0) !== 0 && e.getEndDate() - c !== 0;
                if (h) {
                    e.setStartDate(b, g, false)
                }
                if (g) {
                    e.setEndDate(c, false, false)
                }
                e.endEdit()
            }
        ...
        function(m) {
            ...
            if (l) {
                ...
                case a.EndToEnd:
                    ...
                    break;
                default:
                    throw "Invalid dependency type: " + m.get("Type")
                }
            }
        });
        return {
    ...
    cascadeChanges: function(a, b) {
        ...
        Ext.each(this.getOutComingDependencies(),
        function(c) {
            var d = a.getNodeById(c.get("To"));
            if (d && !d.get("ManuallyScheduled")) {
...
Ext.define("Gnt.data.TaskStore", {
    ...
    isValidDependency: function(d, b, e) {
        var f = true;
        if (d === b) {
            f = false
        }
        ...
        if (f && c.contains(a) || a.contains(c)) {
            f = false
        }
        if (f && this.hasTransitiveDependency(b, d)) {
            f = false
        }
        return f
    },
    ...
    onTaskUpdated: function(c, b, a) {
        if (!this.cascading && (a == Ext.data.Model.EDIT && (b.isStartOrEndModified() || !b.dirty)) || a == Ext.data.Model.REJECT) {
    ...
    cascadeChangesForTask: function(a) {
        ...
        Ext.each(a.getOutComingDependencies(),
        function(d) {
            var e = c.getNodeById(d.get("To"));
            if (e) {
                if (!c.cascading) {
        ...
        if (l) {
            ...
            for (var f = 0; f < b; f++) {
                ...
                if (c) {
                    ...
                    n.registerNode(c);
                    c.commit();
                    Ext.each(this.dependencyStore.getNewRecords(),
                    function(i) {
                        var p = i.get("From");
                        var o = i.get("To");
                        if (p === c._phantomId) {
...
Ext.define("Gnt.feature.DragCreator", {
    ...
    getProxy: function() {
        if (!this.proxy) {
            this.proxy = this.template.append(this.ganttView.el, {},
            true)
        }
        return this.proxy
    },
            ...
            if (k && g) {
                ...
                if (m && h) {
                    ...
                    case b.EndToEnd:
                        ...
                        break;
                    default:
                        throw "Invalid dependency type: " + a.get("Type")
                    }
                    if (p) {
                        d.push({
                            lineCoordinates: p,
...
Ext.define("Gnt.view.Dependency", {
    ...
    renderAllDependenciesBuffered: function() {
        if (this.renderAllDepsBuffered) {
            return
        }
        this.renderAllDepsBuffered = true;
        var a = this;
        setTimeout(function() {
            a.renderAllDepsBuffered = false;
    ...
    onDependencyDrop: function(d, b, a, c) {
        if (this.taskStore.isValidDependency(b, a, c)) {
...
Ext.define("Gnt.view.Gantt", {
    ...
    renderTask: function(i) {
        ...
        m;
        if (j) {
            var s = i.getEndDate() || Sch.util.Date.add(j, Sch.util.Date.DAY, 1),
            g = l.getStart(),
            f = l.getEnd(),
    ...
    onBeforeExpand: function(d, b, c) {
        var e = this,
        a;
        if (!e.rendered || !e.animate) {
            return
        }
        if (e.getNode(d)) {
            a = e.getAnimWrap(d);
            if (!a) {
                a = d.animWrapNormal = e.createAnimWrap(d);
    ...
    onBeforeCollapse: function(d, b, c) {
        ...
        if (e.getNode(d)) {
            a = e.getAnimWrap(d);
            if (!a) {
                a = d.animWrapNormal = e.createAnimWrap(d, c)
            } else {
                if (a.expanding) {
    ...
    onExpand: function(c) {
        ...
        e.slideIn("t", {
            ...
            listeners: {
                scope: d,
                lastframe: function() {
                    b.el.insertSibling(f.query(d.itemSelector), "before");
                    b.el.remove();
                    d.resetScrollers();
                    delete b.record.animWrapNormal;
                    delete a[h];
                    d.fireEvent("afterexpand", d)
                }
            }
        });
        b.isAnimating = true
...
Ext.define("Gnt.panel.Gantt", {
    ...
    fixSelectionMode: function() {
        ...
        Ext.apply(a, {
            onSelectChange: function(l, h, p, f) {
                var n = this,
                q = n.views,
                j = q.length,
                o = n.store,
                ...
                m = h ? "select": "deselect",
                k = 0;
                if ((p || n.fireEvent("before" + m, n, l, g)) !== false && f() !== false) {
                    for (; k < j; k++) {
                        if (!this.__preventUpdateOf || q[k].__lockedType != this.__preventUpdateOf) {
                            if (h) {
                                q[k].onRowSelect(g, p)
                            } else {
                                q[k].onRowDeselect(g, p)
                            }
                        }
                    }
                    if (!p) {
...
Ext.define("Gnt.column.Assign", {
    extend: "Ext.grid.column.Column",
    alias: "widget.assigncolumn",
    header: "Assign",
    dataIndex: "Assign",
    width: 80,
    format: "0",
...
Ext.define("Gnt.column.Feedback", {
    extend: "Ext.grid.column.Column",
    alias: "widget.statuscolumn",
    header: "Notes",
    dataIndex: "Feedback",
...
Ext.define("Gnt.widget.AssignmentGrid", {
    ...
    loadTaskAssignments: function(d) {
        ...
        var e = this.assignmentStore.queryBy(function(g) {
            return g.data.TaskId === d
        });
        e.each(function(h) {
            var g = b.findRecord("ResourceId", h.data.ResourceId, 0, false, true, true);
            if (g) {
                g.set("Units", h.data.Units);
                g.set("Id", h.data.Id);
    ...
    onSelect: function(g, f) {
        ...
        f.each(function(i) {
            ...
            if (h > 0) {
                ...
                if (j) {
                    d[j] = true;
                    a.getById(j).set("Units", h)
                } else {
                    c.push(Ext.create(a.model, {
                        TaskId: e,
...
Ext.define("Gnt.widget.Calendar", {
    ...
    initComponent: function() {
        ...
        Ext.each(this.calendar.getHolidaysRanges(this.startDate, this.endDate),
        function(c) {
